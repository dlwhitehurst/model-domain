...
trait AccountActivity {
   ...
   * transaction history
   */
  val transactions:List[AccountTransaction] = List(new AccountTransaction(new Money ("0.00", FinancialConstants.NUMERIC_DEC, 2, "USD"),1))
  
  def debitAccount(value:Money,acctType:Int):Money
  
  def creditAccount(value:Money,acctType:Int):Money
